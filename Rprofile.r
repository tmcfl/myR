...
.Last <- function() {
    ...
    installedpkgs <- as.vector(allpkgs[is.na(allpkgs[,"Priority"]), 1])
    save(allpkgs, installedpkgs, file="~/Projects/R/installed_packages.RData")
...
# Helper functions
.env$trim_numeric <- function(x, n_decimals=4) { as.numeric(substr(x, start=1, stop=n_decimals+2)) }
...
.env$pct_na_ <- function(x) { cnt_na(x) / length(x) }
# .env$pct_unique_ <- function(x) { round(cnt_unique_(x) / length(x), 5) }
.env$pct_unique_ <- function(x) { cnt_unique_(x) / length(x) }
...
.env$dfcounts <- function(df, include_vis = TRUE) {
  ...
  output$pct_uniq <- trim_numeric(output$cnt_uniq / (nrow_df - output$cnt_na))
...
.env$dfcounts_ <- function(df, include_pct = TRUE, include_na = TRUE, include_vis = TRUE) {
  ...
  if (include_pct) {
    ...
    if (include_na) {
      ...
      output$pct_na <- trim_numeric(output$cnt_na / nrow(df))
      if(include_vis) {
...
.env$count_na <- function(df, include_unique = FALSE, return_only_na_cols = FALSE){
  ...
  if(isTRUE(include_unique)) {
    ...
    output[["pct_unique"]] <- trim_numeric(pct_unique)
  }
...
.env$count_unique <- function(df, include_na = FALSE) {
  ...
  output <- 
    dplyr::tibble(
      ...
      , pct_unique = trim_numeric(pct_unique)
    )
  ...
  if(isTRUE(include_na)) {
    ...
    output[["pct_na"]] <- trim_numeric(pct_na)
  }
...
.env$find_constants <- function(df, include_NAs = FALSE) {
  counts <- dfcounts(df, include_vis = FALSE)
  
  if (isTRUE(include_NAs)) {
    output <- subset(counts, cnt_uniq == 1 & cnt_na > 0)
    output
  } else {
    output <- subset(counts, cnt_uniq == 1 & cnt_na == 0)
    output[, 1:5]
...
rm(updated, user_current_lib_path, profile_message)
